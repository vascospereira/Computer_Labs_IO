...
// static -> not visible outside of this file
...
static void print_usage(char **argv) {
	printf("Usage: one of the following:\n"
			"\t service run %s -args \"config <decimal number - timer>\"\n"
			"\t service run %s -args \"square <decimal number - frequency>\"\n"
			"\t service run %s -args \"int <decimal number - time>\"\n",
...
static int proc_args(int argc, char **argv) {
	...
	if (strncmp(argv[1], "config", strlen("config")) == 0) {
		...
		printf("timer: %s - no such function!\n", argv[1]);
		return 1;
	}
}
...
static unsigned long parse_ulong(char *str, int base) {
	...
	val = strtoul(str, &endptr, base); /* Tries to convert string to unsigned long */
